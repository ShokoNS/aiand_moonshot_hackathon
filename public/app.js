...
async function refreshApiKeyStatus() {
  try {
    ...
    if (payload.kimiConfigured) {
      ...
      status.textContent = "未接続 / Not connected";
...
function demoAnalysis() {
  ...
  if (!app.setupTime) missingQuestionsEn.push("Please tell us the time required for setup and teardown.");
  if (!app.sdsSupport) missingQuestionsEn.push("Please select the support you would like SDS to provide for the co-hosted event.");
  if (!app.alcohol) missingQuestionsEn.push("If food or beverages will be served, please confirm whether alcohol will be provided.");
  return {
    mode: "demo",
    ...
    replyDraftEn: `Thank you for your application.\n\nBased on the details received, ${availableDates.map((date) => formatDate(date)).join(" or ")} are currently available candidate dates.${date1Blocked ? ` Please note that ${formatDate(app.firstDate)} overlaps with an existing booking, so we are proposing an alternative date.` : ""}\n\n${missingQuestionsEn.length ? `To proceed, please confirm the following:\n- ${missingQuestionsEn.join("\n- ")}\n\n` : ""}We will follow up with the final date and event conditions after our internal review.`,
  };
}
...
function renderAnalysis(a) {
  ...
  return `<div class="detail-grid"><div class="detail-card"><div class="section-heading"><h3>SDSとの適合性整理 / SDS fit assessment</h3><span class="mode-tag ${a.mode === "kimi" ? "kimi" : ""}">${a.mode === "kimi" ? "KIMI分析 / KIMI analysis" : "Demo分析 / Demo analysis"}</span></div><div class="fit-layout"><div class="fit-score" style="background:conic-gradient(var(--coral) 0 ${Math.round((a.fitScore || 0) * 3.6)}deg,#f0f1f4 ${Math.round((a.fitScore || 0) * 3.6)}deg 360deg)"><strong>${a.fitScore}</strong><small>適合度 / Fit</small></div><div class="fit-copy"><p>${escapeHtml(a.fitSummary || a.recommendedReason)}</p><ul class="bullet-list">${a.fitReasons.map((reason) => `<li>${escapeHtml(reason)}</li>`).join("")}</ul></div></div></div><div class="detail-card date-card"><div class="section-heading"><h3>空き候補日 / Candidate dates</h3><span class="section-kicker">FACILITY CHECK</span></div><div class="date-list">${dates}</div><div class="alternative-list">${alternatives}</div></div><div class="detail-card"><div class="section-heading"><h3>推奨する追加質問 / Recommended follow-up</h3><span class="section-kicker">${a.missingQuestions?.length || 0}件 / items</span></div><div class="questions">${questionHtml}</div></div><div class="detail-card"><div class="section-heading"><h3>会員・パートナー候補 / Member & partner candidates</h3><span class="section-kicker">匿名概要 / Anonymous profiles</span></div><div class="member-list">${members}</div></div><div class="detail-card"><div class="section-heading"><h3>運営タスク / Operations tasks</h3><span class="section-kicker">${a.tasks?.length || 0}件 / items</span></div><div class="task-list">${tasks}</div></div><div class="detail-card"><div class="section-heading"><h3>返信案 / Reply draft</h3><button class="link-btn" data-action="copy-reply">日英をコピー / Copy both</button></div><div class="reply-pair"><section class="reply-language"><div class="reply-language-label">日本語 / Japanese</div><div class="reply-box" id="replyDraft">${escapeHtml(a.replyDraft)}</div></section><section class="reply-language"><div class="reply-language-label">English / 英語</div><div class="reply-box" id="replyDraftEn">${escapeHtml(a.replyDraftEn || "English reply draft is not available yet.")}</div></section></div><div class="reply-footer"><button class="link-btn" data-action="edit-reply">返信案を編集する / Edit reply →</button></div></div></div>`;
}

function renderRecommendation(a) {
  const isKimi = a.mode === "kimi";
  const tasks = (a.tasks || []).slice(0, 3);
  const availableDates = (a.candidates || []).filter((candidate) => candidate.available).length;
  const missingQuestions = a.missingQuestions?.length || 0;
  const memberMatches = a.memberMatches?.length || 0;
  const taskHtml = tasks.length ? tasks.map((task, index) => `<div class="recommendation-task"><span class="task-number">${index + 1}</span><div><strong>${escapeHtml(task.label)}</strong><small>${escapeHtml(ownerLabel(task.owner))} · ${escapeHtml(priorityLabel(task.priority))}</small></div></div>`).join("") : `<div class="recommendation-empty">運営確認へ進めます。 / Ready for operator review.</div>`;
  return `<div class="detail-card recommend-card"><div class="recommendation-head"><div><span class="recommendation-eyebrow">AI DECISION SUPPORT / AI判断補助</span><h3>次にやること / Recommended next step</h3></div><span class="recommendation-badge ${isKimi ? "kimi" : ""}">${isKimi ? "✦ KIMI提案 / KIMI recommendation" : "デモ提案 / Demo recommendation"}</span></div><div class="recommendation-main"><div class="recommendation-summary"><span>推奨 / Recommendation</span><strong>${escapeHtml(a.recommendedAction || "運営確認へ進む")}</strong><p>${escapeHtml(a.recommendedReason || "申込内容を確認し、運営担当者の判断へ進みます。")}</p></div><div class="recommendation-stats"><div><strong>${availableDates}</strong><span>候補日 / dates</span></div><div><strong>${missingQuestions}</strong><span>追加質問 / questions</span></div><div><strong>${memberMatches}</strong><span>会員候補 / matches</span></div></div></div><div class="recommendation-next"><span class="recommendation-label">まずやること / Start here</span><div class="recommendation-task-list">${taskHtml}</div></div></div>`;
}
